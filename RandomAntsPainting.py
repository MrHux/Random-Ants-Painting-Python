...
class MainWindow():
    ...
    def start(self):
        ...
        for i in range(0, self.nb_fourmis) :
            ...
            colorR = (np.random.randint(0, 255)+(i*100))%255
            colorG = (np.random.randint(0, 255) + (i * 100)) % 255
            colorB = (np.random.randint(0, 255) + (i * 100)) % 255
            color = (colorR, colorG, colorB)
            #déclaration d'une fourmi avec ses coordonnées x,y et sa couleur
            self.fourmis.append(Fourmi(np.random.randint(0, width_image), np.random.randint(0, height_image),color))

        # déclarer les threads
        self.thread1 = Thread_Deplacer_Fourmis(self)
        self.thread2 = Thread_Deplacer_Fourmis(self)
        self.thread3 = Thread_Deplacer_Fourmis(self)
        self.thread4 = Thread_Deplacer_Fourmis(self)

        #thread chargé du rafraichissement de l'affichage
        self.thread_update = Thread_Update_Canvas(self)
...
class Thread_Update_Canvas(threading.Thread):
    def __init__(self, window):
        threading.Thread.__init__(self)
        self.window = window # on lui passe la fenêtre
...
class Thread_Deplacer_Fourmis(threading.Thread):

    #constructeur de la thread
    def __init__(self, window):
        threading.Thread.__init__(self)
        self.window = window # on lui passe la fenêtre afin qu'elle puise récupérer les fourmis et l'image PIL
            ...
            for i in range(0,self.window.nb_fourmis):
                ...
                try:
                    ...
                    if(have_it):
                        ...
                        fourmi.deplacer()
                        #on paint un pixel de l'image la ou se trouve la fourmi
